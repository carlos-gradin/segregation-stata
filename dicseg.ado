*! Program to compute two-groups segregation
*! Carlos Gradin
*! This version 2.3, March 2026
*! Changes from v2.2: bug fixes (missing backticks in aggregated rn, duplicate tempname RT,
*!   error-without-exit for wrong variable count and non-binary groups, silent negative censoring,
*!   graph typo), efficiency improvements (precomputed cumulative sum for Gini, simplified
*!   zero-count check, streamlined index/value filling)
...
* It requires to have matsort.ado (written by Paul Millar) previously installed
	...
	* It also draws the segregation curves if option sc is specified, and creates variables using x() and y() options
		* First variable indicates units (ex. occupations, census tracts, schools, ....), second variable identifies groups (gender, race, ...)


cap program drop dicseg
program def dicseg	 , rclass  byable(recall) sortpreserve
version 10
...
* FIX: removed duplicate RT from tempname list
tempname F z1 z2 D M aux E H G A CV KM Gg seg CT RT FC FR FF S SS index value
marksample touse
...
if "`aggregate'" == "" {
	...
	* FIX: exit with error code when 3 variables specified (was showing error but continuing)
	if "`error'" ~= "" {
...
if "`aggregate'" ~= "" {
	...
	local rr = rowsof(`F')
	mat `aux'  = `F''*J(`rr',1,1)
	* FIX: added missing backticks around `aux' in second term (was: aux[2,1])
	local rn = `aux'[1,1]+`aux'[2,1]
}


* FIX: exit with error code when group variable does not have exactly 2 groups
*      (was showing error but continuing, causing crashes downstream)
if `rc' > 2 | `rc'<2 {
...
* EFFICIENCY: count zeros directly from matrix instead of gen/replace/sum loop

local z1_count = 0
local z2_count = 0
forvalues j = 1 / `rr' {
	if `F'[`j',1]==0 {
		local z1_count = `z1_count' + 1
	}
	if `F'[`j',2]==0 {
		local z2_count = `z2_count' + 1
	}
}
scalar `z1' = `z1_count'
scalar `z2' = `z2_count'
...
mat `G'  = J(`rr',1,0)
	...
	* EFFICIENCY: Precompute reverse cumulative sum of FC[,1] to avoid O(n^2) inner loop
	*   Old: inner loop summed FC[j,1] for j=i+1..rr at each i
	*   New: single backward pass, then O(n) main loop

tempname cumFC
mat `cumFC' = J(`rr',1,0)
if `rr' >= 2 {
	mat `cumFC'[`rr',1] = 0
	forvalues i = `=`rr'-1'(-1)1 {
...
* EFFICIENCY: fill index and value directly without redundant range+loop
*   range already produces 1..rs; value filled from matrix in same loop
...
* FIX: warn user instead of silently replacing negative values with 0
local has_neg = 0
forvalues i=1/`rs' {
	if `seg'[`i',1] < 0 {
		local has_neg = 1
...
if `has_neg' {
	di as text "(Note: some index values were negative due to numerical precision; set to 0)"
}

lab def `index' 1 "Dissimilarity" 2 "Karmel-MacLachlan" 3 "GE(-2)" 4 "GE(-1)" 5 "GE(0)" 6 "GE(.10)" 7 "GE(.25)" 8 "GE(.50)" 9 "GE(.75)" 10 "GE(.90)" 11 "GE(1)" 12 "GE(2)" 13 "Squared root" 14 "Coef. of Variation" 15 "A(.10)" 16 "A(.25)" 17 "A(.50)" 18 "A(.75)" 19 "A(.90)" 20 "A(1)" 21 "A(2)" 22 "A(4)" 23 "Mutual Information" 24 "Gini"
lab val `index' `index'
...
if "`sc'" ~= "" {
	...
	if "`graph_options'"== "" {
		* FIX: corrected typo "45 dgree line" -> "45 degree line"
		local graph_options "aspectratio(1) plotr(m(zero)) connect(l) lpattern(solid) lwidth(medium) lcolor(red) xtick(0(.1)1) xlabel(0(.1)1) legend( cols(3) forcesize label(1 "45 degree line") label(2 "segregation curve") ) ytick(0(.1)1) ylabel(0(.1)1) xtitle("`xtitle'", size(small) ) ytitle("`ytitle'", size(small) )"
	}
